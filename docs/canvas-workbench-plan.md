...
首轮纳管 Claude Code 与 Codex：两者的自然完成都有官方回执，且回执带 CLI 自己的会话与回合标识。Codex 在首个回合之前不写任何记录，新会话要先用一次引导回合认领身份。**回合被停止后两者都不报告任何终态**，取消路径因此仍阻塞；Grok 也尚未找到可靠信号。
...
- 中断请求区分停止投递与中断实际回合，并等待可验证的终态。发送停止键不自动等于已经停止；取消后旧结果不能被当成有效任务完成。缺少可验证终态时保持不确定，见[行为契约](native-control.md)记录的未解决项。
